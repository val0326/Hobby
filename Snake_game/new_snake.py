...
def main():
    ...
    while game_state["program_running"]:
        ...
        # 2. изменить game state исходя из событий
...
def update_game_state(events, game_state):
    ...
    if game_state["game_running"] and not game_state["game_paused"]:
        ...
        check_apple_consumosion(game_state)
...
def check_collisions(game_state):
    ...
    if (
        ...
        or x >= SIZE_X
        or y >= SIZE_Y
        or len(game_state["snake"]) > len(set(game_state["snake"]))
    ):
        game_state["game_running"] = False


def check_apple_consumosion(game_state):
    apples_eaten = 0
    for apple in game_state["apples"]:
        if apple == game_state["snake"][0]:
            ...
            game_state["score"] += 1
            apples_eaten += 1
            game_state["game_speed"] = game_state["game_speed"] + SPEED_CHANGE
...
def draw_walls(screen):
    ...
    pygame.draw.rect(
        screen,
        WALL_COLOR,
        ((0, HEIGHT - wall_size), (WIDTH, HEIGHT)),
    )
    pygame.draw.rect(
        screen,
        WALL_COLOR,
        ((WIDTH - wall_size, 0), (WIDTH, HEIGHT)),
    )
